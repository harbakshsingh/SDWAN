...
# Setting level at environ.environ to prevent unwanted debug messages from AIDE module
LOGGING_CONFIG = '''
{
    "version": 1,
    "formatters": {
...
def main():
    ...
    cli_parser.add_argument('--pid', metavar='<pid>', action=EnvVar, required=False,
                            envvar='CX_PID', type=non_empty_type,
                            help='CX project id, can also be defined via CX_PID environment variable. '
    ...
    cli_parser.add_argument('--version', action='version',
                            version=f'Sastre-Pro Version {version}. Catalog: {catalog_size()} configuration items, '
    ...
    try:
        if is_api_required:
            ...
            with Rest(base_url, cli_args.user, cli_args.password, cli_args.tenant, timeout=cli_args.timeout, verify=False) as api:
    ...
    aide_thread = threading.Thread(
        target=submit_aide_stats, kwargs={'pid': cli_args.pid, 'estimated_savings': task.savings}, daemon=True
    )
    aide_thread.start()
