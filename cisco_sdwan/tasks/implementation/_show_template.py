...
class TaskShowTemplate(Task):
    ...
    def parser(task_args, target_address=None):
        ...
        vals_mutex = values_parser.add_mutually_exclusive_group()
        vals_mutex.add_argument('--regex', metavar='<regex>', type=regex_type,
                                help='regular expression matching device template names or IDs to inspect')
        vals_mutex.add_argument('--not-regex', metavar='<regex>', type=regex_type,
                                help='regular expression matching device template names or IDs NOT to inspect')
        ...
        refs_mutex = refs_parser.add_mutually_exclusive_group()
        refs_mutex.add_argument('--regex', metavar='<regex>', type=regex_type,
                                help='regular expression matching feature template names to include')
        refs_mutex.add_argument('--not-regex', metavar='<regex>', type=regex_type,
                                help='regular expression matching feature template names NOT to include')
    ...
    def runner(self, parsed_args, api: Optional[Rest] = None) -> Union[None, list]:
        ...
        result_tables = parsed_args.subtask_handler(self, parsed_args, api)
    ...
    def values_table(self, parsed_args, api: Optional[Rest]) -> List[Table]:
        ...
        matched_item_iter = (
            ...
            if (issubclass(item_cls, DeviceTemplate) and
                (regex is None or regex_search(regex, item_name, item_id, inverse=parsed_args.regex is None)))
        )
    ...
    def references_table(self, parsed_args, api: Optional[Rest]) -> List[Table]:
        ...
        for item_id, item_name in device_index:
            ...
            for feature_id in device.feature_templates:
                ...
                if feature_info is None:
                    self.log_warning('Template %s references a missing feature template: %s', item_name, feature_id)
                    continue
        ...
        regex = parsed_args.regex or parsed_args.not_regex
        matched_item_iter = (info for info in feature_dict.values()
                             if regex is None or regex_search(regex, info.name, inverse=parsed_args.regex is None))
        for feature_info in matched_item_iter:
            ...
            # Sorted device_templates to ensure the order is always consistent so it doesn't break diff comparison
            is_first = True
